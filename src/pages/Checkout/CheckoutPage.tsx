...
import Menu from '../../components/Header/menus/Menu'
...
const CheckoutPage: React.FC = () => {
	...
	const handleStepComplete = (stepData: any, stepName: keyof CheckoutData) => {
		if (stepName === 'contact') {
			// Отдельно обрабатываем addressId из данных контакта
			const { addressId, ...contactData } = stepData
			setCheckoutData(prev => ({
				...prev,
				contact: contactData,
				addressId: addressId // Сохраняем addressId в корне объекта
			}))
		} else {
			setCheckoutData(prev => ({
				...prev,
		...
		<div>
			...
			<div className='container  mx-auto px-4 py-8 mt-20'>
				<div className='flex flex-col items-center justify-center md:flex-row gap-8'>
					<div className='w-full mt-[20px] md:mt-1 md:mb-[65px] md:w-2/3'>
						...
						{step === 2 && (
							<DeliveryMethod onComplete={data => handleStepComplete(data, 'delivery')} />
						)}
						{step === 3 && <Payment checkoutData={checkoutData} />}
					</div>
					{/* 
