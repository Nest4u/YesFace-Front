...
export const authAPI = {
  async register(userData: any) {
    try {
      ...
      throw error.response?.data?.error || { 
        message: error.response?.data?.message?.[0]?.messages?.[0]?.message || 'Registration failed' 
      }
    }
  },
  ...
  async login(identifier: string, password: string) {
    try {
      ...
      if (response.data.jwt) {
        ...
        initializeUserCart(); // Initialize cart after login
      }
  ...
  logout() {
    try {
      // Очищаем только локальное хранилище
      localStorage.clear(); // Очищаем всё хранилище, включая корзину
      
      // Устанавливаем начальное состояние без dispatch
      const emptyState = {
        items: [],
        status: 'idle',
        userId: null,
